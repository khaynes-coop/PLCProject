...
while(tokens.has(0) && (peek("VAL") | peek("VAR") | peek("LIST") | peek("LET"))) {
        ...
        if(tokens.has(0)){
            throw new ParseException("Globals after Functions", tokens.index);
        }
    ...
    public Ast.Global parseList() throws ParseException {
        ...
        if(peek(Token.Type.IDENTIFIER)){
            ...
            else{throw new ParseException("No Type Specified", tokens.index);}
            if(match("=")){
                if(peek("[")){
                    //match("[");
                    if(peek("[", "]")){
